...
LAYOUT DE OBSERVACIÓN (735 dimensiones):
  ...
  obs[240:480] → LIDAR scan dinámico (240 rayos, [0,1]) — solo vehículos y peatones.
                 Separación semántica estático/dinámico para reducir zigzag inducido
                 por guardarraíles (Khalil et al. [96], Aghdasian et al. [81]).
  obs[480:720] → LIDAR scan estático (240 rayos, [0,1]) — solo obstáculos estáticos
                 (muros, quitamiedos, postes). Contraste útil con dinámico; señal
                 presente desde stage 0 del currículo (sin NPCs).
  ...
  obs[728:730] → Vehicle state (speed_norm, steering)
  obs[730:735] → Route info extendida (5 dims):
...
class CarlaEnv(gym.Env):
    ...
    def _build_observation(self) -> Tuple[np.ndarray, Dict]:
        ...
        Retorna obs (735,) e info enriquecido con datos CARLA para los shields.
        """
        # LIDAR scans (combined=240, dynamic=240, static=240)
