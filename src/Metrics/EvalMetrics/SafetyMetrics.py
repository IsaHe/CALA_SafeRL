...
class SafetyMetrics:
    ...
    def shield_intervention_analysis(infos: List[Dict]) -> Dict[str, Any]:
        ...
        total = len(infos)
        interventions = sum(
            1 for i in infos if i.get("shield_activated", i.get("shield_active", False))
        )
    ...
    def hidden_unsafe_state_detection(
        ...
        for ep in episodes:
            for i, info in enumerate(ep):
                ...
                total_checked += 1
                future_unsafe = any(
                    f.get("shield_activated", f.get("shield_active", False))
                    or f.get("min_distance", 1.0) < 0.10
                    or f.get("collision", False)
                    for f in future
                )
                if future_unsafe:
                    detected += 1
    ...
    def horizon_effectiveness(infos: List[Dict]) -> Dict[int, Dict]:
        data: Dict[int, Dict] = defaultdict(
    ...
    def lane_safety_metrics(infos: List[Dict]) -> Dict[str, float]:
        ...
        lat_offsets = np.array(
            [
                abs(i.get("lateral_offset", i.get("lateral_offset_norm", 0.0)))
                for i in infos
            ]
        )
    ...
    def lane_edge_metrics(infos: List[Dict]) -> Dict[str, float]:
        ...
        left_dists = np.array(
            [i.get("dist_left_edge_norm", 0.5) for i in infos], dtype=np.float32
        )
        right_dists = np.array(
            [i.get("dist_right_edge_norm", 0.5) for i in infos], dtype=np.float32
        )
    ...
    def semantic_lidar_metrics(infos: List[Dict]) -> Dict[str, Any]:
        ...
        road_edge_detection_rate = sum(1 for d in road_edge_dists if d < 999.0) / max(
            n, 1
        )
        road_edge_close_rate = sum(1 for d in road_edge_dists if d < 5.0) / max(n, 1)
        mean_road_edge_pts = float(
            np.mean([i.get("n_road_edge_pts", 0) for i in infos])
        )
        ...
        return {
            ...
            "mean_nearest_pedestrian_m": float(np.mean(ped_real))
            if ped_real
            else 999.0,
            ...
            "mean_nearest_road_edge_m": float(np.mean(road_edge_real))
            if road_edge_real
            else 999.0,
    ...
    def shield_semantic_analysis(infos: List[Dict]) -> Dict[str, Any]:
        ...
        veh_at_intervention = [
            ...
            for i in infos
            if i.get("shield_activated", False)
            and i.get("nearest_vehicle_m", 999.0) < 999.0
        ]
        ped_at_intervention = [
            i.get("nearest_pedestrian_m", 999.0)
            for i in infos
            if i.get("shield_activated", False)
            and i.get("nearest_pedestrian_m", 999.0) < 999.0
        ]
        ...
        return {
            ...
            "mean_vehicle_dist_at_intervention": float(np.mean(veh_at_intervention))
            if veh_at_intervention
            else 999.0,
            "mean_pedestrian_dist_at_intervention": float(np.mean(ped_at_intervention))
            if ped_at_intervention
            else 999.0,
        }
