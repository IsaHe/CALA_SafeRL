...
class SafetyMetricsReporter:
    ...
    def generate_report(
        ...
        if sem_shield and sem_shield.get("total_interventions", 0) > 0:
            ...
            lines.append(
                f"  Static   (guardrail): {sem_shield['static_interventions']:>5}  ({sem_shield['static_rate']:.1%})"
            )
            lines.append(
                f"  Pedestrian:           {sem_shield['pedestrian_interventions']:>5}  ({sem_shield['pedestrian_rate']:.1%})"
            )
        ...
        if sem:
            ...
            lines.append(
                f"  Nearest vehicle    (mean): {mv:.1f} m"
                + (" (no vehicles)" if mv >= 999.0 else "")
            )
            lines.append(
                f"  Nearest pedestrian (mean): {mp:.1f} m"
                + (" (no pedestrians)" if mp >= 999.0 else "")
            )
            lines.append(
                f"  Nearest road edge  (mean): {mr:.1f} m"
                + (" (not detected — check sensor FOV)" if mr >= 999.0 else "")
            )
            lines.append(
                f"  Vehicle   <5m rate:  {sem.get('vehicle_critical_5m_rate', 0):.1%}"
            )
            lines.append(
                f"  Pedestrian <4m rate: {sem.get('pedestrian_critical_4m_rate', 0):.1%}"
            )
            lines.append(
                f"  Road edge detection: {sem.get('road_edge_detection_rate', 0):.1%}  "
        ...
        if lane:
            ...
            lines.append(
                f"  Mean lateral offset: {lane.get('mean_lateral_offset', 0):.3f}"
            )
            lines.append(
                f"  Max lateral offset:  {lane.get('max_lateral_offset', 0):.3f}"
            )
            lines.append(
                f"  >half-lane rate:     {lane.get('pct_above_half_lane', 0):.1%}"
            )
            lines.append(
                f"  Lane invasions:      {lane.get('total_lane_invasions', 0)}"
            )
        ...
        if edge:
            ...
            lines.append(
                f"  Mean min-edge dist:  {edge.get('mean_min_edge_dist', 0):.3f}  "
                f"(1.0=center, 0.0=edge)"
            )
            lines.append(
                f"  Mean left  edge:     {edge.get('mean_left_edge_dist', 0):.3f}"
            )
            lines.append(
                f"  Mean right edge:     {edge.get('mean_right_edge_dist', 0):.3f}"
            )
            lines.append(f"  Alert zone  (<0.30): {edge.get('pct_below_0_3', 0):.1%}")
            lines.append(f"  Critical    (<0.15): {edge.get('pct_below_0_15', 0):.1%}")
            lines.append(
                f"  Mean asymmetry:      {edge.get('mean_edge_asymmetry', 0):.3f}  "
        ...
        if spd:
            lines.append("\nSPEED METRICS:")
            lines.append(
                f"  Mean: {spd.get('mean_speed_kmh', 0):.1f} km/h"
        ...
        if comp and "compliance_rate" in comp:
            ...
            lines.append(
                f"  Compliance rate:      {comp.get('compliance_rate', 0):.1%}"
            )
            lines.append(
                f"  Overspeed >5%  rate:  {comp.get('overspeed_5pct_rate', 0):.1%}"
            )
            lines.append(
                f"  Overspeed >20% rate:  {comp.get('overspeed_20pct_rate', 0):.1%}"
            )
            lines.append(
                f"  Mean speed / limit:   {comp.get('mean_speed_vs_limit', 0):.2f}x"
            )
