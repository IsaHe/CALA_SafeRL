...
methods such as A2C, TRPO~\cite{schulman2015trpo} and PPO~\cite{schulman2017ppo} all build on
Equation~\eqref{eq:pg}; they differ mainly in how they constrain the update step size.
...
over-exploit a good action beyond \(1+\epsilon\); for a negative advantage it cannot
...
sampled rollouts. A plain Monte Carlo estimate has zero bias but high variance; a one-step
...
\begin{equation}
  ...
  \label{eq:gae}
\end{equation}
where \(\lambda \in [0,1]\) is the GAE parameter. Setting \(\lambda = 0\) recovers the one-step
TD advantage; \(\lambda = 1\) recovers the Monte Carlo advantage. The value \(\lambda = 0.95\)
used in this work provides a practical bias-variance balance. When an episode terminates
(collision, off-road), the critic bootstrap value is set to zero; when an episode is truncated by
a step-count timeout, the critic value at the final observed state is used to approximate the
missing return tail.
...
\begin{itemize}
  ...
  \item \textbf{Lagrangian and primal-dual methods.} A more scalable approach forms the dual
        ...
        updates on the policy and the multiplier. Variants underpin much of modern Safe RL.
  ...
  \item \textbf{Runtime shielding.} A verifier inspects each proposed action and substitutes a
        ...
        temporal-logic specifications; Jansen et al.~\cite{jansen2020shielded} extend it to
...
\begin{itemize}
  ...
  \item \textbf{Predefined towns.} Town04 is a highway loop with multi-lane segments and
        entrance ramps; it is the main benchmarking map used in
        Chapter~\ref{ch:experimentation}. Towns 01--03 cover urban scenarios; Town05 offers
        maximum difficulty.
\end{itemize}
...
on CARLA; notable examples include Chen et al.~\cite{chen2019carlarl} on urban driving and
...
Shielding has been applied to benchmarks such as HighwayEnv and CommonRoad. Krasowski et
