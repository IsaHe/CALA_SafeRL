...
\begin{enumerate}
  ...
  \item \textbf{Shield-dependence study (Chapter~\ref{ch:dependence}).} An instrument to measure
        ...
        behavioural-cloning scheme that clones the shield's steering corrections does, producing an
        agent whose shield-off success rate (\(66\%\)) matches its shield-on rate (\(66\%\)). The
        accompanying deterministic safety governor is a reusable ``proposer\,+\,veto'' pattern for
        placing a learned controller inside a safety-critical loop.
  \item \textbf{LLM-derived reward study (Chapter~\ref{ch:llmreward}).} An offline pipeline that
        distils a Large Language Model's pairwise safety preferences into a dense reward model with
        the Bradley\textendash Terry objective, delivered to PPO as policy-invariant potential-based
        shaping, together with an exposure director and a \texttt{log\_std} anchor that stabilises
        warm-started training. Two by-products are of independent value: a \emph{perception-range
        finding}, that the three-channel LIDAR caps vehicle detection at \(\approx12\,\mathrm{m}\)
        and thereby starves any collision signal, fixed by raising the LIDAR to \(16\) channels
        (detection \(\approx71\,\mathrm{m}\), encounter coverage \(3\%\rightarrow34\%\)) at unchanged
        observation dimensionality, and a rigorously de-confounded \emph{negative result}: the
        learned reward improves lane-keeping (mean lateral offset \(0.095\rightarrow0.040\)) but
        trades off-road events for collisions without a net safety gain, locating the remaining gap
...
in Chapter~\ref{ch:experimentation}. The key numbers are: adaptive shield achieves
...
\begin{description}
  ...
  \item[\textbf{Gate the alive bonus by speed.}] An unconditional alive bonus, even a small one,
        ...
        \(0.15\)/step forever. Gating the alive bonus by a speed factor, so that it is
  ...
  \item[\textbf{The shield teaches steering, not throttle.}] A safety shield is an excellent
        ...
        rather than a \emph{scheduling} problem: loosening the shield's thresholds, whether by hand
        or by an LLM, optimises the wrong variable.
  \item[\textbf{Perception range gates the learning signal.}] Much of what looked like a
        ...
        the three-channel LIDAR detecting vehicles only at \(\approx12\,\mathrm{m}\), collision-
        relevant experience occurred in just \(3\%\) of episodes, so no amount of reward design
        could help. Raising the LIDAR to \(16\) channels lifted both detection range and encounter
        coverage roughly tenfold and was the single most decisive change. In an embodied agent a
        learning signal is only ever as good as the perception that feeds it.
...
\begin{itemize}
  ...
  \item \textbf{Longitudinal control for collision avoidance.} Chapter~\ref{ch:llmreward} attacked
        ...
        for collisions, because the captions it was trained on described lateral geometry richly and
        closing dynamics poorly, so the reward carried no braking authority. The clearest
        continuation is therefore to make the reward encode the quantity it currently
        misses (captioning the closing speed or time-to-collision explicitly so the
