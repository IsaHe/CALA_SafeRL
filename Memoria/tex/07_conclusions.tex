...
\begin{enumerate}
  ...
  \item \textbf{Semantic 739-dimensional observation space.} A sensor pre-processing stage
        ...
        reduces the zig-zag pathology introduced by static guardrails.
  \item \textbf{PPO agent with modern stabilisers.} An implementation of PPO with Generalised
        ...
        estimator applied only to the vector block of the observation.
  \item \textbf{Basic safety shield.} A lightweight, interpretable shield that combines LIDAR
        ...
        follows the minimal-interference principle: its override action is zero whenever the
  ...
  \item \textbf{Adaptive-horizon safety shield.} A richer shield that classifies the current
        ...
        pedestrian emergency branch, a lane-change relaxation rule and a threshold-floor safeguard that
  ...
  \item \textbf{Dense reward shaper.} A reward wrapper with twelve additive components plus an
        alive bonus, all gated by speed when appropriate to avoid the degenerate
  ...
  \item \textbf{Shield-dependence study (Chapter~\ref{ch:dependence}).} An instrument to measure
        dependence, the shield-off evaluation probe and the dynamic/static reliance split,
        together with two strategies to reduce it. A Large-Language-Model meta-controller that
        loosens the shield is shown, with data, \emph{not} to reduce dependence. A
        behavioural-cloning scheme that clones the shield's steering corrections does reduce it,
        producing an agent whose shield-off success rate (\(66\%\)) matches its shield-on rate
        (\(66\%\)). The accompanying deterministic safety governor is a reusable
        ``proposer\,+\,veto'' pattern for placing a learned controller inside a safety-critical
        loop.
  \item \textbf{LLM-derived reward study (Chapter~\ref{ch:llmreward}).} An offline pipeline that
        distils a Large Language Model's pairwise safety preferences into a dense reward model
        with the Bradley\textendash Terry objective, delivered to PPO as policy-invariant
        potential-based shaping, together with an exposure director and a \texttt{log\_std}
        anchor that stabilises warm-started training. Two by-products are of independent value:
        a \emph{perception-range finding} showing that the three-channel LIDAR caps vehicle
        detection at \(\approx12\,\mathrm{m}\) and thereby starves any collision signal, fixed
        by raising the LIDAR to \(16\) channels (detection \(\approx71\,\mathrm{m}\), encounter
        coverage \(3\%\rightarrow34\%\)) at unchanged observation dimensionality, and a
        rigorously de-confounded \emph{negative result}: the learned reward improves lane-keeping
        (mean lateral offset \(0.095\rightarrow0.040\)) but trades off-road events for collisions
        without a net safety gain, locating the remaining gap in longitudinal control.
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    SO7 \textendash\ Live metrics pipeline
         ...
         & 8
         & Achieved. \\
    SO8 \textendash\ Measure \& reduce shield dependence
         & \ref{ch:dependence}
         & 10
         & Achieved. \\
    SO9 \textendash\ LLM-derived reward for avoidance
         & \ref{ch:llmreward}
         & 11
         & Achieved (negative result). \\
    \bottomrule
  \end{tabular}
\end{table}

The quantitative evaluation that anchors \emph{``Achieved''} for SO3, SO4 and SO6 is provided
in Chapter~\ref{ch:experimentation}. The key numbers are: the adaptive shield achieves
\(70\%\) success (\(\gamma=60\%\) threshold), \(2\%\) crash (\(\alpha=4\times\) threshold),
and \(0\%\) rolling off-road (\(\beta=15\%\) threshold) as reported in
Table~\ref{tab:config-comparison}.
...
have saved considerable time if they had been understood earlier in the process.
...
\begin{description}
  ...
  \item[\textbf{Gate the alive bonus by speed.}] An unconditional alive bonus, even a small one,
        ...
        \(0.15\)/step indefinitely. Gating the alive bonus by a speed factor, so that it is
        earned only while the vehicle is actually moving, closes this trap without
        hurting exploration.
  \item[\textbf{Give the agent a grace period after a shield override.}] A brake-based shield
        ...
        Without this condition, an always-active shield would perpetually reset the grace period
        and silence the idle penalty indefinitely.
  \item[\textbf{Prefer in-place updates to reconnection.}] Every early attempt at changing the
        ...
        take effect at the next \texttt{reset} is both simpler and more robust.
  ...
  \item[\textbf{The shield teaches steering, not throttle.}] A safety shield is an excellent
        runtime \emph{filter} but an uneven \emph{teacher}: its steering corrections demonstrate
        good lane keeping and can be cloned, but its throttle output is always an emergency brake,
        and cloning it collapses the policy to a stationary equilibrium
        (Section~\ref{sec:dep-collapse}). More broadly, shield dependence proved to be a
        \emph{learning} problem (the agent never received gradient on the shielded states)
        rather than a \emph{scheduling} problem. Loosening the shield's thresholds, whether by
        hand or by an LLM, addresses the wrong variable.
  \item[\textbf{Perception range gates the learning signal.}] Much of what looked like a
        reward-design problem in Chapter~\ref{ch:llmreward} was in fact a sensor-range problem: with
        the three-channel LIDAR detecting vehicles only at \(\approx12\,\mathrm{m}\), collision-relevant
        experience occurred in just \(3\%\) of episodes, so no amount of reward design could help.
        Raising the LIDAR to \(16\) channels lifted both detection range and encounter coverage
        roughly tenfold and was the single most decisive change. In an embodied agent, a learning
        signal is only ever as good as the perception that feeds it.
  \item[\textbf{Experience\,$>$\,gradient\,$>$\,incentive.}] Across
        Chapters~\ref{ch:dependence}--\ref{ch:llmreward} the interventions that worked supplied
        \emph{experience} (exposure, perception) or a \emph{gradient} (behavioural cloning).
        Pure \emph{incentives} (a hand-tuned penalty, a loosened threshold, even an LLM-learned
        reward) only ever reshaped behaviour the agent had already learned. An incentive can refine
        a skill the agent can already execute, but it cannot install a skill (here, longitudinal
...
\begin{itemize}
  ...
  \item \textbf{Urban benchmarks.} The evaluation has been concentrated on Town04, a highway
        ...
        traffic lights and dense pedestrian traffic that would stress-test the pedestrian-emergency
        branch of the adaptive shield. Adapting the curriculum to this richer scenario is itself
        a research question.
  ...
  \item \textbf{Constrained policy optimisation.} PPO is used here as a black-box optimiser of
        the shaped reward. A closer integration with Safe RL would replace reward shaping
        with explicit safety constraints and solve the resulting Constrained MDP with CPO or a
  ...
  \item \textbf{Longitudinal control for collision avoidance.} Chapter~\ref{ch:llmreward} attacked
        the residual collision gap with a dense LLM-derived preference reward and reached a
        de-confounded negative result: the reward improved lane-keeping but traded off-road events
        for collisions, because the captions it was trained on described lateral geometry richly
        and closing dynamics poorly, so the reward carried no braking authority. The clearest
        continuation is to make the reward encode the quantity it currently misses by captioning
        the closing speed or time-to-collision explicitly, so that the
        Bradley\textendash Terry distillation can learn a longitudinal ordering, or to pair the
        learned reward with an explicit longitudinal-control prior. A multi-seed replication would
        also tighten the confidence intervals around the two-point effect reported there.
